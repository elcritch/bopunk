...
def createLocalItem(feed, filename, item_dict={}):
...
class PListList(list):
    def __init__(self, path, feed):
        """init list with plist from given path.
        
        This just simplifies storing manaul firmware list items as a plist.
        It operates as a normal list with initialization from plist or
        ...
        try:
            with open(self.__path,'r') as plist_fl:
                items = [] 
                for elem in plistlib.readPlist(plist_fl):
                    #convert time to time_struct
                    elem['updated_parsed'] = time.gmtime(elem['updated_parsed'])
                    item = FeedItem(feed, None)
                    item.elem = elem
                    item.islocal = True
                    items.append(item)
            ...
            print "FirmwareFeed:PlistError:init:", io
            list.__init__(self)
    ...
    def sync(self):
        ...
        try:
            with open(self.__path,'w') as plist_fl:
                print "FirmwareFeed:PListList:sync: Writing ", self.__path
                items = [ i.elem for i in self ]
                for item in items:
                    #convert time to floating point
                    item['updated_parsed'] = time.mktime(item['updated_parsed'])
                
                print "FirmwareFeed:PListList:sync: list ", items
                plistlib.writePlist(items,plist_fl)
        
        except (IOError), io:
            print "FirmwareFeed:PListList:sync: Error", io

class FirmwareFeed:
    """Parses atom feeds into a python data structure and stores the resulting data."""
    
    def __init__(self, url, mainwindow=None):
...
class FeedItem:
    ...
    def __init__(self, parent, elem):
        ...
        self.elem = elem
        self.parent = parent
        # self._path = path # TODO: depricate
        self.islocal = False

    
    def __getPath(self): # TODO: depricate
